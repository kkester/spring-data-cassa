...
import static io.pivotal.cassa.board.OwnedType.FOR_SALE;
import static io.pivotal.cassa.board.SquareType.*;
import static io.pivotal.cassa.board.db.ColorType.*;
...
public class SquareRepository {
    ...
    public SquareEntity findByName(String name) {
        return BOARD_MAP.values().stream()
                .filter(squareEntity -> squareEntity.getName().equals(name))
                .findFirst()
                .orElse(null);
    }
}
